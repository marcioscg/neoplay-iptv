...
class Storage {
  ...
  // ---------- controle parental ----------
  String? get parentalPin => _p.getString('parental_pin');

  Future<void> saveParentalPin(String? pin) async {
    if (pin == null || pin.isEmpty) {
      await _p.remove('parental_pin');
    } else {
      await _p.setString('parental_pin', pin);
    }
  }
  ...
  List<AdminUser> get adminUsers {
    final raw = _p.getString('admin_users');
    if (raw == null || raw.isEmpty) {
      final defaultUsers = [
        AdminUser(
          id: 'user_demo_1',
          name: 'Cliente Demo',
          email: 'cliente@miaunet.com',
          password: '123',
          plan: AdminPlan.anual,
          createdAt: DateTime.now(),
          expiresAt: DateTime.now().add(const Duration(days: 365)),
          isActive: true,
          notes: 'Usuário padrão demonstrativo',
        ),
      ];
      _p.setString(
        'admin_users',
  ...
  Future<void> saveAdminUsers(List<AdminUser> users) async {
    await _p.setString(
      'admin_users',
  ...
  Map<String, PlaybackProgress> get playbackProgress {
    final raw = _p.getString('playback_progress');
  ...
  Future<void> savePlaybackProgress(PlaybackProgress progress) async {
    ...
    if (progress.fraction >= 0.95 || progress.positionSeconds < 10) {
      map.remove(progress.itemId);
    } else {
      map[progress.itemId] = progress;
    }
    final raw = jsonEncode(map.map((k, v) => MapEntry(k, v.toJson())));
    await _p.setString('playback_progress', raw);
  }
